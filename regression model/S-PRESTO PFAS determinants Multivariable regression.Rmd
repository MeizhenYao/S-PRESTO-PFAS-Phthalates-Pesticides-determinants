...
PFAS_multireg1 <-
   c(lapply(PFAS_cont, function(x){
    ...
    glm(formula = substitute(i ~ ethnicity_specified_recat + pcv1_parity_recat + pcv1_household_income_recat + occupation + Fast_food_sc + fruit_sc, 
                           list(i = as.name(x))),
      data = PFAS_log,
      family = gaussian())
    }),
...
final_model1<- PFAS_multireg1_coefs_data %>% 
...
##### extract results
write_csv(final_model1, "C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/Documents/Projects/S-PRESTO/code/R/chemical & covariates/plot_data/final_model1.csv")
...
PFAS_multireg1 <-
   c(lapply(PFAS_cont, function(x){
    ...
    glm(formula = substitute(i ~ ethnicity_specified_recat + pcv1_parity_recat + pcv1_household_income_recat + occupation + Fast_food_sc + fruit_sc + mca_dim1_cat + mca_dim2_cat, 
                           list(i = as.name(x))),
      data = PFAS_log,
      family = gaussian())
    }),
...
final_model2<- PFAS_multireg1_coefs_data %>% 
                            inner_join(PFAS_info, by="PFAS_name")%>% 
                            mutate(Estimate = if_else(beta=="log(OR)", exp(Estimate), Estimate),
                                   conf.low = if_else(beta=="log(OR)", exp(conf.low), conf.low),
                                   conf.high = if_else(beta=="log(OR)", exp(conf.high), conf.high)) %>% 
                filter(PFAS_name == "PFNA_log")

## put result into one table
tabel<- flextable(as_grouped_data(final_model2, groups = "PFAS_name")) %>%          
        color(~sig == "*", ~conf.low, color = "red")%>% 
        color(~sig == "*", ~conf.high, color = "red")%>%
        theme_box() 
...
##### extract results
write_csv(final_model2, "C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/Documents/Projects/S-PRESTO/code/R/chemical & covariates/plot_data/final_model2.csv")
...
# ####### extract results
PFAS_model1<- rbind(model1,
                    parity)


write_csv(PFAS_model1, "C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/Documents/Projects/S-PRESTO/code/R/chemical & covariates/plot_data/PFAS_model1_raw.csv")
...
PFAS_multireg1 <-
   c(lapply(PFAS_cont, function(x){
    ...
    glm(formula = substitute(i ~ fruit_sc + ethnicity_specified_recat + age_at_recruitment_cat + pcv1_highest_education_completed_recat + pcv1_household_income_recat + occupation, 
                           list(i = as.name(x))),
      data = PFAS_log,
      family = gaussian())
    }),
...
PFAS_multireg1 <-
   c(lapply(PFAS_cont, function(x){
    ...
    glm(formula = substitute(i ~ vegetable_sc + ethnicity_specified_recat + age_at_recruitment_cat + pcv1_highest_education_completed_recat + pcv1_household_income_recat + occupation, 
                           list(i = as.name(x))),
      data = PFAS_log,
      family = gaussian())
    }),
...
PFAS_multireg1 <-
   c(lapply(PFAS_cont, function(x){
    ...
    glm(formula = substitute(i ~ Styrofoam_boxes + ethnicity_specified_recat + age_at_recruitment_cat + pcv1_highest_education_completed_recat + pcv1_household_income_recat + occupation, 
                           list(i = as.name(x))),
      data = PFAS_log,
      family = gaussian())
    }),
...
PFAS_multireg1 <-
   c(lapply(PFAS_cont, function(x){
    ...
    glm(formula = substitute(i ~ Plastic_boxes + ethnicity_specified_recat + age_at_recruitment_cat + pcv1_highest_education_completed_recat + pcv1_household_income_recat + occupation, 
                           list(i = as.name(x))),
      data = PFAS_log,
      family = gaussian())
    }),
...
PFAS_multireg1 <-
   c(lapply(PFAS_cont, function(x){
    ...
    glm(formula = substitute(i ~ mca_dim1_cat + ethnicity_specified_recat + age_at_recruitment_cat + pcv1_highest_education_completed_recat + pcv1_household_income_recat + occupation,
                           list(i = as.name(x))),
      data = PFAS_log,
      family = gaussian())
    }),
...
mca_dim1<- PFAS_multireg1_coefs_data %>%
            filter(term %in% c("mca_dim1_catHigh-tendency-outside"))
```
...
PFAS_multireg1 <-
   c(lapply(PFAS_cont, function(x){
    ...
    glm(formula = substitute(i ~ mca_dim2_cat + ethnicity_specified_recat + age_at_recruitment_cat + pcv1_highest_education_completed_recat + pcv1_household_income_recat + occupation,
                           list(i = as.name(x))),
      data = PFAS_log,
      family = gaussian())
    }),
...
mca_dim2<- PFAS_multireg1_coefs_data %>%
            filter(term %in% c ("mca_dim2_catHigh-tendency-packaging"))
